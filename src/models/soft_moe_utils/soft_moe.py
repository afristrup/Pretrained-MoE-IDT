...
from src.models.soft_moe_utils.distributed import (
    AllGather,
    split_by_rank,
    gather_sizes,
    has_only_one_value,
...
def GLUFeedForward(dim, mult=4, dropout=0.0):
...
class Experts(nn.Module):
    def __init__(
        self, experts, is_distributed=None, offload_unused_experts_to_cpu=True
    ):
        super().__init__()
...
class SoftMoE(Module):
    ...
    def forward(self, x, mask=None, add_noise=False, noise_mult=1.0):
        """
        einstein notation
        b - batch
        n - sequence length
