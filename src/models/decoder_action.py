...
class DecoderAction(TrajectoryModel):
    ...
    def forward(self, states, actions, z, timesteps, attention_mask=None):
        ...
        stacked_inputs = (
            torch.stack((z_embeddings, state_embeddings, action_embeddings), dim=1)
            .permute(0, 2, 1, 3)
    ...
    def get_predict_action(
        self, states, actions, z_distributions, timesteps, attention_mask=None
    ):
        z_distributions = z_distributions.reshape(-1, 1, 2 * self.z_dim).repeat(
            1, states.size()[1], 1
        )
