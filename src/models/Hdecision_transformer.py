...
from src.models.trajectory_softmoe import construct_model
...
class HDecisionTransformer(TrajectoryModel):
    def __init__(
        ...
        **kwargs,
        ...
        self.transformer = construct_model(num_experts=4, tokens_per_expert=64, dim=128) # GPT2Model(config)
        ...
        self.predict_z_distribution = nn.Sequential(
            *(
                [nn.Linear(hidden_size, 2 * self.z_dim)]
    ...
    def forward(
        ...
        stacked_inputs = (
            torch.stack((returns_embeddings, state_embeddings, z_embeddings), dim=1)
            .permute(0, 2, 1, 3)
    ...
    def get_action(
        self, states, z_distributions, rewards, returns_to_go, timesteps, **kwargs
    ):
        ...
        if self.max_length is not None:
            ...
            z_distributions = torch.cat(
                [
                    torch.zeros(
                        (
                            z_distributions.shape[0],
                            self.max_length - z_distributions.shape[1],
                            2 * self.z_dim,
                        ),
                        device=z_distributions.device,
                    ),
                    z_distributions,
                ],
                dim=1,
